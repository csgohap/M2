 ...
 *           2] Имя таблицы, которую использует модель
 *           3] Имя столбца с первичным ключём
 *           4] Настройка автоинкремента столбца с первичным ключём
 *           5] Автообслуживание столбцов created_at / updated_at
 *
 *         Дополнительно
 *         -------------
 *           6] Использовать или нет мягкое удаление
 *           7] Настроить "белый список" mass assignment
 *           8] Настроить "черный список" mass assignment
 *           9] Настроить автоприведение типов
 *           10] Настроить список $hidden (исключ.столбцов из результата при преобразовании в массив/json)
 *           11] Настроить список $visible (только столбцы из списка будут в преобразовании в массив/json)
 *           12] Настроить список $appends (список столбцов, которые добавляются к результату при преобразовании модели в массив/json)
 *
 *       В3. Заготовки запросов модели (scopes)
 *
 *         1] Пример заготовки запроса
 *
 *       В4. Связи модели
 *
 *         Связи
 *         -----
 *
 *           1] lognotes       | Связь n:m            | Получить коллекцию связанных с этим тегом записей в логе
 *
...
// В. Класс-модель //
//-----------------//
class MD2_tags extends Model { // implements ModelInterface
  ...
  //------------------//
    ...
    //-------//
        ...
        //-------------//
        // - Связь n:m "прямая"
        // - Получить коллекцию связанных с этим тегом записей в логе
        public function lognotes()
        {
          return $this->belongsToMany('\M2\Models\MD1_log', 'm2.md1000', 'id_tag', 'id_lognote');
        }
